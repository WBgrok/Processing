...
void setup() 
{
  ...
  shot1 = createImage(width, height,RGB);
  shot2 = createImage(width, height,RGB);
  first_shot = true;
  disp_result = false;
  background(0);
}
...
void captureEvent(Capture myCapture) {
  shot1.copy(myCapture, 0, 0,width, height, 0, 0,width, height);
  myCapture.read();
}
...
void draw() {
  PImage disp = blender(myCapture,shot1,DIFFERENCE);
  disp.filter(THRESHOLD);
 image(disp,0,0);
}
